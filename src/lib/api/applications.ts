...
import type { SkillTestDifficulty, SkillTestTemplate } from '@/types/skill-tests';
...
export interface ApplicationWithDetails extends Application {
    ...
    skill_test_attempt_id?: string | null;
    skillTestAttempt?: {
        id: string;
        score: number;
        passed: boolean;
...
export async function getAssignmentApplications(
    ...
    const { data, error } = await supabase
      ...
      *,
      ...
      skillTestAttempt:skill_test_attempts(
        id,
        score,
        passed,
        difficulty,
        time_taken_seconds,
        completed_at,
        template:skill_test_templates(id, name, category)
      )
    `)
