...
import { hasPassedSkillTest } from '@/lib/api/skill-tests';
...
export default function ApplyToGig() {
    ...
    const [checkingSkillTest, setCheckingSkillTest] = useState(false);
    const [skillTestResultId, setSkillTestResultId] = useState<string | null>(null);
    ...
    async function loadGigAndProfile() {
        try {
            setLoading(true);
            ...
            // Check skill test requirement (job seekers only)
            if (role === 'job_seeker' && gigData.requires_skill_test && gigData.skill_test_id) {
                setCheckingSkillTest(true);
                const result = await hasPassedSkillTest(gigData.skill_test_id);
                setSkillTestPassed(result.passed);
                if (result.passed && result.resultId) {
                    setSkillTestResultId(result.resultId);
                }
                setCheckingSkillTest(false);
            } else {
                // Employers exempt or no test required
                setSkillTestPassed(true);
            }
        } catch (err) {
    ...
    return (
        <div className="container max-w-3xl mx-auto px-4 py-8">
            ...
            {!isEmployer && gig.requires_skill_test && gig.skill_test_id && (
                <Card className={`p-4 mb-6 ${skillTestPassed ? 'border-green-500' : 'border-yellow-500'}`}>
                    <div className="flex items-center justify-between">
                        ...
                        {!skillTestPassed && (
                            <Button
                                onClick={() => navigate(`/skill-tests/${gig.skill_test_id}`)}
                                variant="outline"
                            >
                                Take Test
                            </Button>
