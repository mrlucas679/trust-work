                        ...
                        {pendingApplications.map(application => (
                            <Card key={application.id} className="border-l-4 border-l-yellow-500">
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {application.skillTestAttempt && (
                                        <div>
                                            <h4 className="font-semibold mb-2">Skill Test Results</h4>
                                            <div className="bg-primary/5 p-4 rounded-lg border border-primary/20">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-sm font-medium">Test Score:</span>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-2xl font-bold">{application.skillTestAttempt.score}%</span>
                                                        <Badge variant={application.skillTestAttempt.passed ? "default" : "destructive"}>
                                                            {application.skillTestAttempt.passed ? "✓ PASSED" : "✗ FAILED"}
                                                        </Badge>
                                                    </div>
                                                </div>
                                                <div className="grid grid-cols-3 gap-2 text-sm text-muted-foreground">
                                                    <div>
                                                        <span className="block text-xs">Difficulty</span>
                                                        <Badge variant="outline" className="mt-1 capitalize">
                                                            {application.skillTestAttempt.difficulty}
                                                        </Badge>
                                                    </div>
                                                    <div>
                                                        <span className="block text-xs">Time Taken</span>
                                                        <span className="block font-medium mt-1">
                                                            {Math.floor(application.skillTestAttempt.time_taken_seconds / 60)}:{(application.skillTestAttempt.time_taken_seconds % 60).toString().padStart(2, '0')}
                                                        </span>
                                                    </div>
                                                    <div>
                                                        <span className="block text-xs">Completed</span>
                                                        <span className="block font-medium mt-1">
                                                            {new Date(application.skillTestAttempt.completed_at).toLocaleDateString()}
                                                        </span>
                                                    </div>
                                                </div>
                                                {application.skillTestAttempt.template && (
                                                    <p className="text-xs text-muted-foreground mt-2">
                                                        Test: {application.skillTestAttempt.template.name}
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                        ...
                        {acceptedApplications.map(application => (
                            <Card key={application.id} className="border-l-4 border-l-green-500 bg-green-50/50">
                                ...
                                <CardContent className="space-y-3">
                                    ...
                                    {/* Skill Test Score (Accepted) */}
                                    {application.skillTestAttempt && (
                                        <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                                            <div className="flex items-center justify-between text-sm">
                                                <span className="font-medium">Skill Test: {application.skillTestAttempt.score}%</span>
