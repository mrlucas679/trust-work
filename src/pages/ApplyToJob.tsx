...
export default function ApplyToJob() {
    ...
    async function loadJobAndProfile() {
        try {
            ...
            // Check skill test requirement
            if (jobData.requires_skill_test && jobData.skill_test_id) {
                setCheckingSkillTest(true);
                const result = await hasPassedSkillTest(jobData.skill_test_id);
    ...
    return (
        <div className="container max-w-3xl mx-auto px-4 py-8">
            ...
            {/* Skill Test Gate */}
            {job.requires_skill_test && job.skill_test_id && (
                <Card className={`p-4 mb-6 ${skillTestPassed ? 'border-green-500' : 'border-yellow-500'}`}>
                    <div className="flex items-center justify-between">
                        ...
                        {!skillTestPassed && (
                            <Button
                                onClick={() => navigate(`/skill-tests/${job.skill_test_id}`)}
                                variant="outline"
                            >
                                Take Test
                            </Button>
